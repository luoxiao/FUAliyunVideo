...
@interface AliyunMagicCameraViewController () <AliyunMusicPickViewControllerDelegate,UIGestureRecognizerDelegate,UIAlertViewDelegate,AliyunIRecorderDelegate>
...
#pragma clang diagnostic pop
    ...
    if (_quVideo.encodeMode == AliyunEncodeModeSoftH264) {
...
//    NSLog(@"高级美颜：faceunity调用,磨皮:%f,,美白:%f,红润:%f,大眼:%f,瘦脸:%f",self.blurValue,self.beautyWhiteValue,self.buddyValue,self.bigEyeValue,self.slimFaceValue);
    ...
    if ([[FUMusicPlayer sharePlayer] isPlaying]) {
        [[AlivcShortVideoFaceUnityManager shareManager] musicFilterSetMusicTime:[FUMusicPlayer sharePlayer].currentTime];
    }

...
- (instancetype)initWithBackgroundImage:(UIImage *)image{
    ...
    if (self) {
        ...
        //计算大小 - 宽度是整个屏幕的0.26 高度根据图片比例计算
        CGFloat width = [UIScreen mainScreen].bounds.size.width * 0.26;
        ...
        self.progressView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
        [self addSubview:self.progressView];
        
        //标题
        UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-30, self.bounds.size.width, 30)];
        textLabel.text = @"准备中";
...
- (void)setProgress:(CGFloat)progress{
    ...
    [UIView animateWithDuration:0.1 animations:^{
        self.progressView.center = CGPointMake(self.frame.size.width / 2, cy);
...
- (void)setShowText:(NSString *)text{
    ...
    self.textLabel.center = CGPointMake(self.frame.size.width / 2, self.frame.size.height - 30);

...
+ (void)loginRandomUserWithHandler:(void (^)(NSString *userId, NSString *token,
                                             NSError *error))handler {
  [self getWithPath:@"/user/randomUser"
                 params:nil
...
+ (void)getVideoUploadAuthWithWithTitle:(NSString *)title
                               ...
                               coverURL:(NSString *_Nullable)coverURL
                                ...
                                handler:(void (^)(NSString *, NSString *,
                                                  NSString *,
                                                  NSError *))handler {
  ...
  [self getWithPath:@"/demo/getVideoUploadAuth"
                 params:params
      completionHandler:^(NSURLResponse *response, id responseObject,
                          NSError *error) {
        if (error) {
          ...
          NSString *uploadAddress =
              [responseObject objectForKey:@"uploadAddress"];
...
+ (void)refreshVideoUploadAuthWithVideoId:(NSString *)videoId
  ...
  [self getWithPath:@"/demo/refreshVideoUploadAuth"
                 params:params
      completionHandler:^(NSURLResponse *response, id responseObject,
                          NSError *error) {
        if (error) {
...
+ (void)getImageUploadAuthWithTitle:(NSString *_Nullable)title
  ...
  [self getWithPath:@"/demo/getImageUploadAuth"
                 params:params
      completionHandler:^(NSURLResponse *response, id responseObject,
                          NSError *error) {
        if (error) {
  ...
  NSURLSessionDataTask *task = [session
      dataTaskWithRequest:urlRequest
        completionHandler:^(NSData *_Nullable data,
          ...
          if (code != 200) {
            NSError *error = [NSError errorWithDomain:@"AliyunSVideoApi"
                                                 code:code
                                             userInfo:jsonObj];
            if (completionHandler) {
              completionHandler(response, nil, error);
...
+ (NSString *)getParamsString:(NSDictionary *)params {
  ...
  for (id key in params.allKeys) {
    ...
    NSString *part =
        [NSString stringWithFormat:@"%@=%@", [self percentEncode:key],
                                   [self percentEncode:value]];
    [parts addObject:part];
  }

  NSArray<NSString *> *sortedArray = [parts
...
+ (NSString *)percentEncode:(id)object {
  ...
  NSString *const kAFCharactersGeneralDelimitersToEncode =
  ...
  NSString *percentstring = [string
      stringByAddingPercentEncodingWithAllowedCharacters:allowedCharacterSet];
  NSString *plusReplaced =
      [percentstring stringByReplacingOccurrencesOfString:@"+"
                                               withString:@"%20"];
  NSString *starReplaced =
      [plusReplaced stringByReplacingOccurrencesOfString:@"*"
                                              withString:@"%2A"];
  NSString *waveReplaced =
      [starReplaced stringByReplacingOccurrencesOfString:@"%7E"
                                              withString:@"~"];
  return waveReplaced;
}
@end

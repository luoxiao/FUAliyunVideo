...
-(NSString *)getHintForItemStr:(NSString *)str{
    return [self.hintDic objectForKey:str];
}
...
- (void)loadItem:(NSString *)itemName{
    ...
    if (itemName != nil && ![itemName isEqual: @"noitem"]) {
        ...
        if (isPortraitDrive || isAnimoji) {
            ...
            [FURenderer itemSetParam:itemHandle withName:@"isFlipTrack" value:@(value)];
            [FURenderer itemSetParam:itemHandle withName:@"isFlipLight" value:@(value)];
        }
...
-(void)musicFilterSetMusicTime:(NSTimeInterval)timeInterval{
    [FURenderer itemSetParam:items[1] withName:@"music_time" value:@(timeInterval * 1000 + 50)];//需要加50ms的延迟
}
